...
android {
    defaultConfig {
        applicationId "com.xbh.whiteboard"
    }
    productFlavors {
        xbhSdk5 {
            dimension "default"
...
**示例**：
- `./gradlew testXbhSdk5ReleaseUnitTest`
...
**示例**：
- `./gradlew assembleXbhSdk5DebugAndroidTest`
...
**示例**：
```bash
adb install app/build/outputs/apk/androidTest/xbhSdk5/debug/app-xbhSdk5-debug-androidTest.apk
```
...
**示例**：
```bash
adb shell am instrument -w -e package com.xbh.whiteboard com.xbh.whiteboard.test/androidx.test.runner.AndroidJUnitRunner
```
...
### 代码规范
1. **注释规范**：所有生成的测试代码必须按照XBH_AI_PATCH规范添加注释
...
3. 执行testXbhSdk5ReleaseUnitTest
4. 处理错误并重测，直到通过
5. 询问用户是否继续AndroidTest
6. 用户确认后，生成AndroidTest用例（LoginActivity测试）
7. 执行assembleXbhSdk5DebugAndroidTest
8. 安装androidTest APK
...
```bash
./gradlew testXbhSdk5ReleaseUnitTest --tests "com.example.SpecificTest"
```
