...
#### 完整命令示例
```bash
./gradlew assemblexxxDebug --parallel --configure-on-demand --build-cache
```
...
3. 解析并列出所有assemble变体
4. 等待用户选择
5. 执行用户选择的构建任务，包含优化参数：`./gradlew assemblexxxDebug --parallel --configure-on-demand --build-cache`
6. 返回构建结果或错误信息

**注意**：所有gradle命令都使用 `./gradlew`，不要使用 `gradlew.bat`

### 示例2：指定具体变体
用户输入：`打包assemblexxxDebug`

处理流程：
1. 识别用户指定的变体：`assemblexxxDebug`
2. 直接执行构建命令：`./gradlew assemblexxxDebug --parallel --configure-on-demand --build-cache`
3. 返回构建结果或错误信息

### 示例3：描述性指定
用户输入：`构建xxx flavor的release版本`

处理流程：
1. 识别用户意图：xxx flavor + release构建
2. 确定变体：`assemblexxxRelease`
3. 直接执行构建命令：`./gradlew assemblexxxRelease --parallel --configure-on-demand --build-cache`
4. 返回构建结果或错误信息
...
### Q: 可以同时构建多个变体吗？
A: 可以，用户可以指定多个变体，例如："构建xxx和demo的debug版本"，skill会依次执行构建。
...
```
设置后，当用户未指定变体时，将自动构建 `assemblexxxDebug`

**添加快速构建别名**：
```json
{
  "buildAliases": {
    "快速打包": "assemblexxxDebug",
    "发布包": "assemblexxxRelease",
...
#### 3. 配置文件优先级
配置文件的优先级高于交互式选择：
1. 如果用户明确指定了变体（如"打包assemblexxxDebug"），使用用户指定的
2. 如果用户未指定变体：
...
```json
{
  ...
  "buildAliases": {
    "开发包": "assemblexxxDebug",
    "调试包": "assemblexxxDebug"
  }
}
```
...
1. **默认变体选择**
   ...
   - 如果配置了 `defaultFlavor: "xxx"` 和 `defaultBuildType: "Debug"`，则默认构建 `assemblexxxDebug`
